...
class PollCommand extends CConsoleCommand
{
  ...
  private function processNewDebugInfoFiles()
  {
    ...
    foreach($debugInfoFiles as $debugInfo)
    {
      ...
      $matches = array();
      $check = preg_match(
        '#Assync command \{([0-9]{1,9}.[0-9]{1,9})\} has been added to the request queue.#',
        $responce, $matches
      );
  ...
  public function checkAssyncCommand($cmdId, &$cmdRetCode, &$cmdRetMsg)
  {
    ...
    $matches = array();
    $check = preg_match(
        '#Command \{([0-9]{1,9}.[0-9]{1,9})\} returned \{(.+)\}#',
        $responce, $matches);
  ...
  public function importCrashReportFromXml($xmlFileName, $crashReportId)
  {
    ...
    try{
      ...
      if(strlen($sehExceptionCode    )!=0) $crashReport->exception_code      = $sehExceptionCode    ;
      if(strlen($exceptionThreadID   )!=0) $crashReport->exception_thread_id = $exceptionThreadID   ;
      if(strlen($exceptionAddress    )!=0) $crashReport->exceptionaddress    = $exceptionAddress    ;
      if(strlen($exceptionModuleName )!=0) $crashReport->exceptionmodule     = $exceptionModuleName ;
      if(strlen($exceptionModuleBase )!=0) $crashReport->exceptionmodulebase = $exceptionModuleBase ;
      ...
      foreach($doc->StackTrace as $elemStackTrace)
      {
        ...
        foreach($elemStackTrace->Row as $elemRow)
        {
          ...
          if(strlen($symName    )!=0) $stackFrame->symbol_name     = $symName    ;
          if(strlen($undSymName )!=0) $stackFrame->und_symbol_name = $undSymName ;
          if(strlen($offsInSym  )!=0) $stackFrame->offs_in_symbol  = $offsInSym  ;
          if(strlen($srcFile    )!=0) $stackFrame->src_file_name   = $srcFile    ;
          if(strlen($srcLine    )!=0) $stackFrame->src_line        = $srcLine    ;
          if(strlen($offsInLine )!=0) $stackFrame->offs_in_line    = $offsInLine ;
